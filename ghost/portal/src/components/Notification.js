...
const NotificationText = ({type, status, context}) => {
    ...
    if (type === 'signin' && status === 'success' && context.member) {
        ...
        return (
            <p>
                No se pudo iniciar sesión. El enlace de inicio de sesión expiró. <a href={signinPortalLink} target="_parent">Haz click aquí para probar de nuevo</a>
            </p>
        );
        ...
        return (
            <p>
                ¡No hemos podido actualizar el correo electrónico! Enlace no válido.
            </p>
        );
    } else if (type === 'signup' && status === 'error') {
        return (
            <p>
                Error al registrarte: enlace no válido <br /><a href={singupPortalLink} target="_parent">Haz click aquí para probar de nuevo</a>
            </p>
        );
        ...
        if (context.member) {
            return (
                <p>
                    ¡Bien! Tu cuenta está completamente activada, ahora tienes acceso a todo el contenido.
                </p>
            );
        }
        return (
            <p>
                ¡Perfecto! Consulta tu correo electrónico para ver el enlace mágico de inicio de sesión.
            </p>
        );
        ...
        if (context.member) {
            return (
                <p>
                    La actualización del plan se ha cancelado.
                </p>
            );
        }
        return (
            <p>
                El pago del plan se ha cancelado.
            </p>
        );
    }
    return (
