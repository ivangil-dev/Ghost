...
function ShowPaidMemberMessage({site, isPaid}) {
    if (isPaid) {
        return (
            <p style={{textAlign: 'center', marginTop: '12px', marginBottom: '0', color: 'var(--grey6)'}}>Dar de baja los correos electrónicos no cancelará tu suscripción de pago a {site?.title}</p>
        );
    }
    return null;
}
