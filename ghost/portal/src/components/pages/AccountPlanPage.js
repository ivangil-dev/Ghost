...
const Header = ({onBack, showConfirmation, confirmationType}) => {
    ...
    let title = isPaidMember({member}) ? 'Cambiar plan' : 'Elegir plan';
    if (showConfirmation) {
...
const PlanConfirmationSection = ({plan, type, onConfirm}) => {
    ...
    const planStartMessage = `${plan.currency_symbol}${priceString}/${plan.interval} – Empieza el ${planStartDate}`;
    ...
    if (type === 'changePlan') {
        ...
        return (
            <div className="gh-portal-logged-out-form-container gh-portal-cancellation-form">
                <p>Si cancelas tu suscripción ahora, continuarás teniendo acceso hasta el <strong>{getDateString(subscription.current_period_end)}</strong>.</p>
