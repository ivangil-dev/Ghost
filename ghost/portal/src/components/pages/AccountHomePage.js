    ...
    return (
        <footer className='gh-portal-account-footer'>
            <ul className='gh-portal-account-footermenu'>
                <li><button className='gh-portal-btn' name='logout' aria-label='salir' onClick={e => handleSignout(e)}>Cerrar sesión</button></li>
            </ul>
...
function getOfferLabel({offer, price, subscriptionStartDate}) {
    ...
    if (offer) {
        ...
        if (discountDuration === 'forever') {
            ...
            durationLabel = `Acaba ${getDateString(offerEndDate)}`;
        }
    ...
    if (subscriptionHasFreeTrial({sub: subscription})) {
        ...
        return (
            <p className="gh-portal-account-discountcontainer">
                <div>
                    <span>Prueba gratuita – Acaba {trialEnd}</span>
...
const PaidAccountActions = () => {
    ...
    const PlanLabel = ({price, isComplimentary, subscription}) => {
        ...
        if (isComplimentary) {
            if (compExpiry) {
                label = `Complementaria - Finaliza ${compExpiry}`;
            } else {
                label = label ? `Complementaria (${label})` : `Complementaria`;
            }
        }
...
function EmailNewsletterAction() {
    ...
    let label = subscribed ? 'Suscrito' : 'No suscrito';
...
function EmailPreferencesAction() {
    ...
    return (
        <section>
            <div className='gh-portal-list-detail'>
                <h3>Emails</h3>
                <p>Actualizar mis preferencias</p>
            </div>
...
const AccountWelcome = () => {
    ...
    if (subscription) {
        ...
        if (isComplimentary && getCompExpiry({member})) {
            ...
            return (
                <div className='gh-portal-section'>
                    <p className='gh-portal-text-center gh-portal-free-ctatext'>Tu suscripción caducará en {expiryAt}</p>
                </div>
            );
        }
        ...
        if (subscriptionHasFreeTrial({sub: subscription})) {
            ...
            return (
                <div className='gh-portal-section'>
                    <p className='gh-portal-text-center gh-portal-free-ctatext'>Tu suscripción comenzará en {trialEnd}</p>
                </div>
            );
        }
        return (
            <div className='gh-portal-section'>
                <p className='gh-portal-text-center gh-portal-free-ctatext'>Tu suscripción se renovará en {getDateString(currentPeriodEnd)}</p>
            </div>
        );
    }
    ...
    return (
        <div className='gh-portal-section'>
            <p className='gh-portal-text-center gh-portal-free-ctatext'>Actualmente tienes una suscripción gratuita, actualiza a una suscripción de pago para tener acceso completo.</p>
            <SubscribeButton />
        </div>
    );
};
...
const ContinueSubscriptionButton = () => {
    ...
    const CancelNotice = () => {
        ...
        const currentPeriodEnd = subscription.current_period_end;
        return (
            <p className='gh-portal-text-center gh-portal-free-ctatext'>Tu suscripción caducará en {getDateString(currentPeriodEnd)}</p>
        );
    };
